...
func _populate_achievements(unlocked_ids: Array, is_owner: bool):
	# Pulisci la lista precedente
	for child in achievements_container.get_children():
		child.queue_free()

	var all_achievements = AchievementRegistry.get_all_achievements()

	for achievement_id in all_achievements:
		var achievement: Achievement = all_achievements[achievement_id]
		var is_unlocked = achievement_id in unlocked_ids
		
		var hbox = HBoxContainer.new()
		
		var label = Label.new()
		label.text = "%s: %s" % [achievement.title, achievement.description]
